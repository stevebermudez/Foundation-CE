/**
 * Production Course Seeding - Called on server startup
 * This ensures courses exist in both development and production databases.
...
export async function seedProductionCourses() {
  try {
    // Check if courses already exist
    const existingCourses = await db.select().from(courses);
    const existingCount = existingCourses.length;
    ...
    const newCount = finalCourses.length;
    
    if (newCount > existingCount) {
      console.log(`✓ Seeded ${newCount - existingCount} new courses (total: ${newCount})`);
    } else {
      console.log(`✓ All ${newCount} courses verified`);
    }
  } catch (err) {
